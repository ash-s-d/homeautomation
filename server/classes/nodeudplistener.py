...
class NodeUDPListener(NodeListener):
    ...
    def run(self):
        ...
        while True:
            ...
            if data is not None:
                print "Received message from " + src_addr + " on UDP port " + str(self._port) + ": " + data

                if data == self._DISCOVER_CMD:
                    print "Sending Pi Camera info to target client " + src_addr + " on UDP port " + str(recv_port)

                    sock.sendto(self._get_info(), (src_addr, recv_port))
    ...
    def stop(self):
        ...
        sock.sendto(self._STOP_CMD, (self._ip_address, self._port))
